...
from model.utils.pool_queue import PoolQueue
...
def get_adv_current_parameters(model):
...
if __name__ == '__main__':
    ...
    train_summary_file = open(os.path.join(summary_dir, 'train_summary.txt'), 'w')
    val_summary_file = open(os.path.join(summary_dir, 'val_summary.txt'), 'w')

    print('max epochs:{}'.format(args.max_epochs))
    print('start epoch:{}'.format(args.start_epoch))
    print('total epoch:{}'.format((args.max_epochs - args.start_epoch)+1))
    print('iters_per_epoch:{}'.format(iters_per_epoch))

    print(iters_per_epoch * ((args.max_epochs - args.start_epoch) + 1)) 

    pool_queue = PoolQueue(max_queue_size=int(256/args.batch_size))
    ...
    while niter < iters_per_epoch * ((args.max_epochs - args.start_epoch) + 1):
        ...
        if args.use_restarting and (niter == 0 and niter % args.restarting_iters == 0):
            print('Initialize adv. branch!')
            ...
            if niter == 0:
                ...
                for _ in itertools.repeat(None, args.initialize_steps):
                    loss_adv_temp = 0
                    loss_aux_temp = 0
                    acc_altitude_temp = 0
                    acc_angle_temp = 0
                    ...
                    for _ in itertools.repeat(None, init_minibatches):
                        try:
                        ...
                        RCNN_loss_weather, RCNN_loss_weather_adv, RCNN_acc_weather = fasterRCNN(im_data, meta_data, run_partial=True, is_first=True)
                        ...
                        acc_altitude = acc_altitude / init_minibatches
                        loss_altitude_adv = loss_altitude_adv / init_minibatches
                        loss_altitude_aux = loss_altitude / init_minibatches
                        acc_angle = acc_angle /init_minibatches
                        loss_angle_adv = loss_angle_adv / init_minibatches
                        loss_angle_aux = loss_angle / init_minibatches
                        acc_weather = acc_weather / init_minibatches
                        loss_weather_adv = loss_weather_adv / init_minibatches
                        loss_weather_aux = loss_weather / init_minibatches
                    ...
                    print(
                        "Initialization (Auxiliary): [session %d][epoch %2d][iter %4d/%4d] altitude accuracy: %.4f, angle accuracy: %.4f, weather accuracy: %.4f, auxiliary loss: %.4f, adversarial loss: %.4f, lr: %.2e, time:%.4f" \
                        % (args.session, niter // iters_per_epoch + 1, niter % iters_per_epoch, iters_per_epoch,
                        acc_altitude_temp, acc_angle_temp, acc_weather_temp, loss_aux_temp, loss_adv_temp, lr, time.perf_counter()-start))
                ...
                n_queue = len(pool_queue)
                for feat, meta in pool_queue.get_queue():
                    RCNN_loss_altitude, RCNN_loss_altitude_adv, RCNN_acc_altitude, \
                    RCNN_loss_angle, RCNN_loss_angle_adv, RCNN_acc_angle, \
                    RCNN_loss_weather, RCNN_loss_weather_adv, RCNN_acc_weather = fasterRCNN(im_data=feat, meta_data=meta, run_partial=True)
                    ...
                    acc_altitude = acc_altitude / n_queue
                    loss_altitude_adv = loss_altitude_adv / n_queue
                    loss_altitude_aux = loss_altitude / n_queue
                    acc_angle = acc_angle / n_queue
                    loss_angle_adv = loss_angle_adv / n_queue
                    loss_angle_aux = loss_angle / n_queue
                    acc_weather = acc_weather / n_queue
                    loss_weather_adv = loss_weather_adv / n_queue
                    loss_weather_aux = loss_weather / n_queue
                ...
                aux_optimizer.step()
                print(
                    "Restarting (Auxiliary): [session %d][epoch %2d][iter %4d/%4d] altitude accuracy: %.4f, angle accuracy: %.4f, weather accuracy: %.4f, auxiliary loss: %.4f, adversarial loss: %.4f, lr: %.2e, time: %.4f" \
                    % (args.session, niter // iters_per_epoch + 1, niter % iters_per_epoch, iters_per_epoch,
                        acc_altitude_temp, acc_angle_temp, acc_weather_temp, loss_aux_temp, loss_adv_temp, lr, time.perf_counter()-start))
        ...
        if args.use_adversarial_loss:
            ...
            for _ in itertools.repeat(None, args.n_minibatches):
                ...
                rois_label, pool_data = fasterRCNN(im_data=im_data, im_info=im_info, meta_data=meta_data, gt_boxes=gt_boxes, num_boxes=num_boxes, run_partial=False)
                    
                pool_queue.enqueue(pool_data)
            ...
            optimizer.step()
            print(
                "Alternating Training (Utility + Adversarial): [session %d][epoch %2d][iter %4d/%4d] utility loss: %.4f, auxiliary loss: %.4f, adversarial loss: %.4f, utility+adversarial loss: %.4f, lr: %.2e, time: %.4f" \
                % (args.session, niter // iters_per_epoch + 1, niter % iters_per_epoch, iters_per_epoch,
                    loss_util_temp, loss_aux_temp, loss_adv_temp, loss_temp, lr, time.perf_counter()-start))

            train_summary_file.write("[session %d][epoch %2d][iter %4d/%4d] utility loss: %.4f, auxiliary loss: %.4f, adversarial loss: %.4f, utility+adversarial loss: %.4f, lr: %.2e, time: %.4f" \
                % (args.session, niter // iters_per_epoch + 1, niter % iters_per_epoch, iters_per_epoch,
                    loss_util_temp, loss_aux_temp, loss_adv_temp, loss_temp, lr, time.perf_counter()-start))
        ...
        if args.monitor_discriminator:
            ...
            current_adv_params = get_adv_current_parameters(fasterRCNN)
            past_adv_params = copy.deepcopy(current_adv_params)

            feat, meta = pool_queue.get_latest_value()
            RCNN_loss_altitude, RCNN_loss_altitude_adv, RCNN_acc_altitude, \
            RCNN_loss_angle, RCNN_loss_angle_adv, RCNN_acc_angle, \
            RCNN_loss_weather, RCNN_loss_weather_adv, RCNN_acc_weather = fasterRCNN(im_data=feat, meta_data=meta, run_partial=True)
            ...
            loss_altitude_aux = loss_altitude 
            loss_angle_aux = loss_angle
            loss_weather_aux = loss_weather
            ...
            print(
                "Alternating Training (Auxiliary): [session %d][epoch %2d][iter %4d/%4d] altitude accuracy: %.4f, angle accuracy: %.4f, weather accuracy: %.4f, auxiliary loss: %.4f, adversarial loss: %.4f, lr: %.2e, time: %.4f" \
                % (args.session, niter // iters_per_epoch + 1, niter % iters_per_epoch, iters_per_epoch,
                    acc_altitude_temp, acc_angle_temp, acc_weather_temp, loss_aux_temp, loss_adv_temp, lr, time.perf_counter()-start))

            # ema update
            update_network_parameters(args.ema_beta, current_adv_params, past_adv_params)
        ...
        if niter % args.save_iters == 0:
            ...
            print('save model: {} --> time:{}'.format(save_name, cur_time-start))

        niter += 1
    

    save_name = os.path.join(model_dir, 'faster_rcnn_final.pth')
    save_checkpoint({
        'session': args.session,
        'epoch': niter // iters_per_epoch + 1,
        'model': fasterRCNN.module.state_dict() if args.mGPUs else fasterRCNN.state_dict(),
    ...
    os.close(train_summary_file.fileno())
    os.close(val_summary_file.fileno())
